...
> ## Transaction ids — added 2026-08-05, and read the warning
>
> A Tessera transaction id is **derived client-side, before signing and before
> submission**: `SHA-256` over the domain-tagged, length-framed *intent* —
> `chain_id, sender, recipient, amount, fee, nonce, asset`.
>
> ⚠️ **`asset` was added 2026-08-12 (T2-012) and every id changed.** It is the
> byte naming which asset the transfer moves: `0` = OMC (Omne Coin, the commerce
> and utility asset), `1` = OGT (Omne Governance Token — voting rights, staked,
> and transferable). It is **inside the signed preimage on purpose**: without
> it one signature authorises either asset, and anyone able to flip the byte in
> flight moves the governance token under a signature meant for the coin.
>
> An SDK that omits it computes a pre-selector id and will disagree with the
> node about **every** transaction. All four implementations were updated
> together and assert against the regenerated vectors; the file now carries five
> OMC cases and one OGT case, because covering one asset would mean agreeing
> with the node about half the protocol. The signature and public key
> are **not** in the preimage, which is what lets an SDK show a user the id at
> submit time and be certain the chain will record the same one.
>
> ⚠️ **`hash_transaction` is NOT this, and must not be used for Tessera.** It
> targets the **archived** chain: no domain tag, **no length framing**, and a
> different field set (`gasLimit`/`gasPrice`/`data` instead of `fee`). Raw
> concatenation of a variable-length `data` with an optional `to` is exactly the
> ambiguity the framing rule exists to prevent — two different transactions can
> share a preimage.
>
> Conformance is `vectors/tx_id_v1.json`, generated by the Rust implementation.
> Six cases move every field and sit on the numeric edges (`u128::MAX` amount
> and fee, `u64::MAX` nonce) where a mis-typed integer width surfaces, plus one
> that exercises the OGT path.
> `amount`/`fee`/`nonce` are decimal **strings**, because `u128` does not survive
> JSON's number type and a silently truncated amount would look like an encoding
> bug rather than a parsing one.
>
> | | |
> |---|---|
> | Rust | `node/src/blocks/tx.rs` (`Transaction::id`) — in `OmneDAO/tessera` |
> | TypeScript | `typescript/src/tx-id.ts` |
> | Python | `python/src/omne_sdk/tx_id.py` |
> | Go | `go/txid.go` |
>
> Each implementation **reuses its own package's existing `digest`/framing
> helper** rather than adding a second one. §R0.2 requires exactly one
> implementation of a canonical preimage; two copies can only be kept in step by
> a test that fails *after* someone edits one of them.
